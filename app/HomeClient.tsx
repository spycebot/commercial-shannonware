...
export default function HomeClient({ articles, stories }: {
  ...
  return (
    <div style={{ minHeight: '100vh', ...PAPER }}>
      ...
      {/* Navigation chrome */}
      <HamburgerMenu articles={articles} />
      <GearMenu />
      ...
      <div style={{ maxWidth: 660, margin: '0 auto', padding: '0 28px' }}>
        ...
        {/* ── 7. § 5 Dispatches — stories from the database, newest first ───── */}
        {stories.length > 0 && (
          <section id="dispatches" style={{ padding: '60px 0 0' }}>
            <SectionLabel>§ 5 · DISPATCHES</SectionLabel>
            <BlueRule />
            <div style={{ display: 'flex', flexDirection: 'column', gap: 44 }}>
              {stories.map((s) => (
                <Grow key={s.slug}>
                  {() => (
                    <article>
                      <div style={{ fontFamily: T.mono, fontSize: FS.label, letterSpacing: 1, color: T.blue }}>
                        {s.date} · DISPATCH
                      </div>
                      <h2 style={{ fontFamily: T.body, fontSize: FS.title, fontWeight: 'normal', color: T.ink, margin: '8px 0 14px' }}>
                        {s.title}
                      </h2>
